...
func (h *Handler) RegisterRoutes(router *mux.Router) {
    router.HandleFunc("/storage/items", h.authMiddleware.ProfileAuthHandler(h.handleGetItems)).Methods("GET")
    router.HandleFunc("/storage/items", h.authMiddleware.ProfileAuthHandler(h.handleCreateItem)).Methods("POST")

    router.HandleFunc("/storage/items/{id}", h.authMiddleware.ProfileAuthHandler(h.handleGetItem)).Methods("GET")
    router.HandleFunc("/storage/items/{id}", h.authMiddleware.ProfileAuthHandler(h.handleUpdateItem)).Methods("PUT")
    router.HandleFunc("/storage/items/{id}", h.authMiddleware.ProfileAuthHandler(h.handleDeleteItem)).Methods("DELETE")

    router.HandleFunc("/storage/items/{id}/restore", h.authMiddleware.ProfileAuthHandler(h.handleRestoreItem)).Methods("PUT")
}
    ...
    if strings.Contains(contentType, "multipart/form-data") {
        ...
        if files := r.MultipartForm.File["images"]; len(files) > 0 {
            ...
            for _, fileHeader := range files {
                url, err := s3Handler.UploadFile(fileHeader, profileCtx.FamilyID, fmt.Sprintf("items/%d", itemID))
                if err != nil {
                    writeError(w, http.StatusInternalServerError, err.Error())
                    return
                }
